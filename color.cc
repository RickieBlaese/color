...
std::string ssto::color::out(const color4b_t& color, bool is_fg) {
	return esc + std::to_string(is_fg ? color.fg : color.bg) + 'm';
}

std::string ssto::color::out(const color24b_t& color, bool is_fg) {
	return esc + std::string(is_fg ? "38;2;" : "48;2;") + std::to_string(color.r) + ';' + std::to_string(color.g) + ';' + std::to_string(color.b) + 'm';
}

void ssto::color::gradient(color24b_t color1, color24b_t color2, unsigned int count, std::vector<color24b_t>& outvec) {
	float c = count;
...
void ssto::color::generate_rainbow(unsigned int length, std::vector<color24b_t>& outvec) {
